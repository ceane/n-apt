...
export const isValidStringEnhanced = (
  value: unknown,
  minLength = 0,
  maxLength = Infinity,
): value is string => {
...
export const isValidNumberEnhanced = (
  value: unknown,
  min = -Infinity,
  max = Infinity,
): value is number => {
...
export const isValidUint8ClampedArray = (
  value: unknown,
...
export const validateWaterfallData = (
  value: unknown,
...
export const validateSpectrumData = (
  value: unknown,
  expectedLength?: number,
...
export const validateWaterfallDataComprehensive = (
  ...
  if (options.width && options.height) {
    ...
    if (dataPoints !== expectedLength) {
      errors.push(
        `Data length (${dataPoints}) does not match expected dimensions (${options.width}x${options.height} = ${expectedLength})`,
      );
    }
  }
  ...
  if (options.fftSize && options.width) {
    ...
    if (
      options.width !== options.fftSize &&
      options.width !== options.fftSize / 2
    ) {
      warnings.push(
        `Waterfall width (${options.width}) may not match FFT size (${options.fftSize})`,
      );
    }
  }
  ...
  if (options.sampleRate) {
    ...
    if (options.sampleRate > 10_000_000) {
      // 10 MHz
  ...
  if (options.centerFrequencyHz) {
    ...
    if (options.centerFrequencyHz > 30_000_000_000) {
      // 30 GHz
      warnings.push(
        `Very high center frequency: ${options.centerFrequencyHz} Hz`,
      );
    }
  }
  ...
  if (options.timestamp) {
    ...
    if (Math.abs(timestampMs - now) > 60000) {
      // More than 1 minute off
      warnings.push(
        `Timestamp is significantly off: ${new Date(timestampMs).toISOString()}`,
      );
    }
    ...
    if (timestampMs > now + 5000) {
      ...
      errors.push(
        `Timestamp is in the future: ${new Date(timestampMs).toISOString()}`,
      );
    }
  }
  ...
  if (alphaChannelIssues > 0) {
    warnings.push(
      `Found ${alphaChannelIssues} pixels with non-255 alpha values`,
    );
  }
  ...
  if (!hasColor) {
    ...
    warnings.push(
      `High percentage of black pixels: ${((zeroPixels / (data.length / 4)) * 100).toFixed(1)}%`,
    );
  }
...
export const validateSpectrumDataComprehensive = (
  ...
  if (options.fftSize && dataPoints !== options.fftSize) {
    errors.push(
      `Data length (${dataPoints}) does not match expected FFT size (${options.fftSize})`,
    );
  }
  ...
  if (dataPoints > 0 && (dataPoints & (dataPoints - 1)) !== 0) {
    warnings.push(
      `Data length (${dataPoints}) is not a power of 2, unusual for FFT data`,
    );
  }
  ...
  if (zeroValues === data.length) {
    ...
    warnings.push(
      `High percentage of zero values: ${((zeroValues / data.length) * 100).toFixed(1)}%`,
    );
  }
  ...
  if (Number.isFinite(maxValue) && Number.isFinite(minValue)) {
    ...
    if (dynamicRange === 0) {
      ...
      warnings.push(
        `Very high dynamic range: ${dynamicRange.toFixed(1)} dB - possible gain issue`,
      );
    }
  }
  ...
  if (options.isFirstFrame) {
    ...
    if (zeroValues === data.length) {
      warnings.push(
        "First frame contains all zeros - device may be initializing",
      );
    }
  }
...
export const hasValidIntegrity = (
  data: unknown,
): data is SdrProcessorMetadata => {
  ...
  if (metadata.latency) {
    ...
    if (
      ...
      !isValidNumberEnhanced(processingTimeMs, 0)
    )
      return false;
  }
...
export const isValidWebSocketMessageWithIntegrity = (
  data: unknown,
): boolean => {
...
export const isValidSpectrumFrameEnhanced = (
  data: unknown,
): data is SpectrumFrame => {
...
export const isValidCaptureRequestEnhanced = (
  data: unknown,
...
export const isValidAuthResponse = (
  response: unknown,
): response is AuthResult => {
...
export const isValidSessionToken = (token: unknown): token is string => {
  return (
    isValidStringEnhanced(token, 20, 500) && /^[a-zA-Z0-9+/=_-]+$/.test(token)
  );
};
...
export const isValidError = (
  ...
  return (
    isValidStringEnhanced(err.message, 1, 1000) &&
    (err.code === undefined || isValidStringEnhanced(err.code, 1, 50))
  );
};
