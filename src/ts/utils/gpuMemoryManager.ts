...
export class GPUMemoryManager {
  ...
  private allocations: Map<
    string,
    { size: number; type: "buffer" | "texture"; created: number }
  > = new Map();
  ...
  private updateStats() {
    ...
    const averageAllocationSize =
      this.allocations.size > 0 ? totalAllocated / this.allocations.size : 0;
    const fragmentationRatio =
      averageAllocationSize > 0
        ? Math.max(
            0,
            1 -
              totalAllocated / (this.allocations.size * averageAllocationSize),
          )
        : 0;
  ...
  private checkMemoryThresholds() {
    ...
    if (allocatedMB > this.thresholds.criticalThreshold) {
      console.error(
        `🚨 Critical GPU memory usage: ${allocatedMB.toFixed(1)}MB`,
      );
      this.emergencyCleanup();
  ...
  performCleanup() {
    ...
    if (cleanedCount > 0) {
      console.log(
        `🧹 GPU Memory cleanup: removed ${cleanedCount} old allocations`,
      );
      this.updateStats();
  ...
  performMaintenance() {
    ...
    const allocatedMB = this.stats.totalAllocated / (1024 * 1024);
    if (allocatedMB > 100) {
      // Only log if significant usage
      console.log(
        `📊 GPU Memory: ${allocatedMB.toFixed(1)}MB (${this.stats.bufferCount} buffers, ${this.stats.textureCount} textures)`,
      );
    }
  }
  ...
  getMemoryUsagePercentage(): number {
    return (
      (this.stats.totalAllocated /
        (this.thresholds.maxAllocation * 1024 * 1024)) *
      100
    );
  }
  ...
  getEfficiencyReport(): {
    ...
    if (this.stats.fragmentationRatio > 0.3) {
      recommendations.push(
        "High memory fragmentation detected - consider buffer pooling",
      );
    }

    if (usagePercent > 80) {
      recommendations.push(
        "Memory usage is high - consider reducing texture sizes or implementing LOD",
      );
    }

    if (this.stats.bufferCount > 100) {
      recommendations.push(
        "Many small buffers - consider consolidating into larger buffers",
      );
    }
