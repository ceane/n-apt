...
export function useAudioExtraction(
  ...
  const applyFiltering = useCallback(
    (audioBuffer: Float32Array, sampleRate: number): Float32Array => {
  ...
  const playAudio = useCallback(
    async (audioBuffer: Float32Array, sampleRate: number): Promise<void> => {
      try {
  ...
  const mixAudioRegions = useCallback(
    (regions: Array<{ buffer: Float32Array; gain: number }>): Float32Array => {
