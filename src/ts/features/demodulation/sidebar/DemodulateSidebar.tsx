...
import { ScanningProgress } from "@n-apt/spectrum";
import { DemodulationMathSidebar } from "@n-apt/demodulation/sidebar/DemodulationMathSidebar";
import { DemodSidebarNodes } from "@n-apt/demodulation/sidebar/DemodSidebarNodes";
import { usePrompt } from "@n-apt/ui/PromptProvider";
import { DemodulationFlows } from "@n-apt/demodulation/sidebar/DemodulationFlows";
...
import { type FlowTemplate } from "@n-apt/demodulation/react-flow/flows";
...
export const DemodulateSidebar: React.FC<DemodulateSidebarProps> = ({
  ...
  const { setFlow, nodes, clearFlow } = useDemodFlow();
  ...
  const handleFlowSelect = useCallback(
    (flow: FlowTemplate) => {
      ...
      if (flow.id === "apt-audio") {
        dispatch(setAlgorithm("fmDiscriminator"));
      }
    },
    [dispatch, setFlow],
  );
