...
import {
  ...
  SIDEBAR_NAV_ROUTES,
} from "@n-apt/spectrum/sidebar/sidebarNavRoutes";
...
/** Heavy route chunks are fetched on hover so navigation is instant. */
const ROUTE_PRELOADERS: Record<string, () => void> = {
  "/demodulate": preloadDemodChunk,
  "/demod": preloadDemodChunk,
};
...
export const SidebarRoutesNav: React.FC<SidebarRoutesNavProps> = ({
  ...
  const handleRouteHover = useCallback((routePath: string) => {
    ROUTE_PRELOADERS[routePath]?.();
  }, []);
        ...
        {visibleRoutes.map((route) => {
          ...
          return (
            <RouteLinkButton
              ...
              onFocus={() => handleRouteHover(route.path)}
