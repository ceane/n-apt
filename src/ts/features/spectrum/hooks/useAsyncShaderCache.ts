...
export function useAsyncShaderCache(options: AsyncShaderCacheOptions) {
  ...
  const createRenderPipeline = useCallback(
    async (
      variant: ShaderVariant,
      variantKey: string,
      ...
      try {
        ...
        if (variant.computeCode) {
          ...
          if (isComputeShader) {
            ...
            pipeline = device.createComputePipeline({
              ...
              compute: {
                module: shaderModule,
                entryPoint: variant.vertexCode.includes("main")
                  ? "main"
                  : variant.vertexCode.includes("compute_main")
                    ? "compute_main"
                    : variant.vertexCode.includes("fft_compute")
                      ? "fft_compute"
                      : "main",
              },
            });
            ...
            const vertexState = {
              module: shaderModule,
              entryPoint: variant.vertexCode.includes("vs_main")
                ? "vs_main"
                : "vs",
            };
            ...
            const fragmentState = variant.fragmentCode
              ? {
                  module: shaderModule,
                  entryPoint: variant.fragmentCode.includes("fs_main")
                    ? "fs_main"
                    : "fs",
        ...
        throw new Error(
          `Failed to create pipeline for ${variantKey}: ${error instanceof Error ? error.message : "Unknown error"}`,
        );
      }
    },
    [device, format],
      ...
      await Promise.allSettled(
        batch.map(async (task) => {
          try {
            ...
            if (pipelineCacheRef.current.size > maxCacheSize) {
              ...
              if (oldestKey) {
                ...
                if (oldPipeline) {
                  ...
                  if (
                    "destroy" in oldPipeline.pipeline &&
                    typeof oldPipeline.pipeline.destroy === "function"
                  ) {
            ...
            setCacheStats((prev) => ({
              ...prev,
              compileCount: prev.compileCount + 1,
            }));
          } catch (error) {
            task.reject(
  ...
  const getPipeline = useCallback(
    (
      ...
      const existingTask = compilationQueueRef.current.find(
        (task) => task.variantKey === variantKey,
      );
      if (existingTask) {
  ...
  const preloadShaders = useCallback(
    async (variants: ShaderVariant[]) => {
  ...
  useEffect(() => {
    return () => {
      ...
      pipelineCacheRef.current.forEach((cached) => {
        if (
          "destroy" in cached.pipeline &&
          typeof cached.pipeline.destroy === "function"
        ) {
