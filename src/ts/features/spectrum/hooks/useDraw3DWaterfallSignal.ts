...
export function useDraw3DWaterfallSignal() {
  ...
  const draw3DWaterfallSignal = useCallback(
    async (options: Draw3DWaterfallSignalOptions): Promise<boolean> => {
      const {
        canvas,
        waveform,
      ...
      try {
        const waveformArray =
          waveform instanceof Uint8Array
        ...
        if (isFirstFrame || isPaused) {
          ...
          if (!validationResult.isValid) {
            console.error(
              `3D waterfall validation failed (${isFirstFrame ? "first frame" : "paused"}):`,
              validationResult.errors,
            );
          } else if (validationResult.warnings.length > 0) {
            console.warn(
              `3D waterfall validation warnings (${isFirstFrame ? "first frame" : "paused"}):`,
        ...
        const usableFrames = Math.max(
          1,
          Math.min(frames.length, Math.floor(plotHeight / 6)),
        );
        ...
        for (let frameIdx = 0; frameIdx < visibleFrames.length; frameIdx++) {
          ...
          const yOffset =
            (visibleFrames.length - 1 - frameIdx) *
            Math.max(4, Number(frameSpacing));
          ...
          for (let i = 0; i < frame.length; i++) {
            ...
            const x =
              plotLeft + xInset + xNorm * Math.max(1, plotWidth - xInset * 2);
        ...
        for (let i = 0; i < frontFrame.length; i++) {
          ...
          const dbNorm =
            (frontFrame[i] - fftMin) / Math.max(1, fftMax - fftMin);
