...
} from "@n-apt/spectrum/utils/rendering/fifoWaterfall2d";
...
export const FIFOWaterfall = memo<FIFOWaterfallProps>(
  ({
    ...
    const lastAppendedWaveformSnapshotRef = useRef<Float32Array | null>(null);
    const hasDrawnWaveformRef = useRef(false);
    const liveDrawRef = useRef<
      ((waveform: Float32Array, appendRow?: boolean) => void) | null
    >(null);
      ...
      for (let i = 0; i < GRADIENT_LUT_SIZE; i++) {
        const color =
          colormap[
            Math.min(
              colormap.length - 1,
              Math.round((i / 255) * (colormap.length - 1)),
            )
          ];
        lut[i * 4] = color[0];
        lut[i * 4 + 1] = color[1];
    ...
    useEffect(() => {
      ...
      if (
        ...
        lastAppendedWaveformSnapshotRef.current = null;
    ...
    useEffect(() => {
      ...
      const drawFrame = (
        ...
        // Prop/effect redraws can be caused by zoom, pan, theme, or layout.
        // Feed notifications explicitly opt into a new history row, including
        // when the producer reuses and mutates the same typed-array object.
        const freezeFrame = freezeAfterPreview || !appendRow;
        if (hasWebGPU && gpuDevice && gpuFormat) {
          const rendered = drawWebGPUFIFOWaterfall({
            ...
            historyZoom: waterfallHistoryFill === "immutable" ? historyZoom : 1,
            historyPan: waterfallHistoryFill === "immutable" ? historyPan : 0,
            binSubset,
          });
          if (!rendered) {
            ...
            if (!freezeFrame) {
              ...
              if (!waveformFeed) {
                lastAppendedWaveformSnapshotRef.current = new Float32Array(
                  renderWaveform,
                );
              }
            }
        ...
        if (!freezeFrame) {
          ...
          const resampled = performScalarResampling(
            selectWaterfallBinSubset(
              renderWaveform,
              binSubset ?? DEFAULT_WATERFALL_BIN_SUBSET,
            ) as any,
            renderWidth,
            scalarResampleOutputRef.current,
          );
      ...
      if (renderWaveform) {
        const lastSnapshot = lastAppendedWaveformSnapshotRef.current;
        const sameWaveformContents =
          lastAppendedWaveformRef.current === renderWaveform &&
          lastSnapshot?.length === renderWaveform.length &&
          lastSnapshot.every((value, index) =>
            Object.is(value, renderWaveform[index]),
          );
        const appendRow =
          !waveformFeed && !sameWaveformContents;
        drawFrame(renderWaveform, appendRow);
      }
    ...
    useEffect(() => {
      ...
      if (current?.length) {
        ...
        liveDrawRef.current?.(current, true);
      }
      return waveformFeed.subscribe((nextWaveform) => {
        ...
        liveDrawRef.current?.(nextWaveform, true);
      });
    }, [waveformFeed]);
