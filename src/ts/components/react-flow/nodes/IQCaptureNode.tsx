...
export const IQCaptureNode = memo(
  ...
  const spectrumRange = useAppSelector((state) => state.spectrum.frequencyRange);
  ...
  const [captureFileType, setCaptureFileType] = useState<CaptureFileType>(".wav");
  ...
  const [customRange, setCustomRange] = useState({ startHz: 0, endHz: 3_200_000 });
  const [hasInitialized, setHasInitialized] = useState(false);
  
  const sampleRateHz = useAppSelector((state) => state.spectrum.sampleRateHz) ?? 3_200_000;
  ...
  }, [customRange, demodCenterFreqHz, spectrumRange, sampleRateHz, isConnectedToChannelNode, effectiveFrames]);
  ...
  const { bandwidthHz, bandwidthCenterFrequencyHz } = useMemo(() => {
    if (activeCaptureAreas.includes("Custom Range")) {
      return {
        ...
        bandwidthCenterFrequencyHz: Math.round((customRange.startHz + customRange.endHz) / 2)
      };
    }
    
    // For other areas (Onscreen or Channels), use the first one as primary for bandwidth metadata
    if (activeCaptureAreas.length > 0) {
      const area = captureRange.segments.find(s => s.label === activeCaptureAreas[0]);
      if (area) {
        return {
  ...
  }, [activeCaptureAreas, customRange, captureRange.segments, sampleRateHz, demodCenterFreqHz]);
  ...
  const handleStartHzChange = useCallback((hz: number) => {
    ...
    setActiveCaptureAreas((current) =>
      current.includes("Custom Range") ? current : [...current, "Custom Range"],
    );
  }, []);
  ...
  const handleEndHzChange = useCallback((hz: number) => {
    setCustomRange(prev => {
      ...
      return {
        startHz: prev.startHz > maxStart ? Math.max(0, maxStart) : prev.startHz,
        endHz: nextEnd
      };
    });
    setActiveCaptureAreas((current) =>
  ...
  const availableCaptureAreasWithExtra = useMemo(() => {
    const baseAreas = availableCaptureAreasFromStore.length > 0
  ...
  const handleCapture = async () => {
    ...
    dispatch(
      sendCaptureCommand({
        ...
        durationS: captureDurationMode === "timed" ? captureDurationS : undefined,
  ...
  return (
    <NodeContainer selected={selected}>
      ...
      <IQCaptureControlsSection
        ...
        onStopCapture={() => dispatch(sendCaptureStopCommand(captureStatus?.jobId))}
