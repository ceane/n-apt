...
const StatusIcon = styled.div<{ $status: "locked" | "unlocked" | "error" }>`
  color: ${props => {
    switch (props.$status) {
      case "unlocked": return props.theme.success || "#4caf50";
      case "error": return props.theme.danger || "#f44336";
      default: return props.theme.textSecondary || "#888";
    }
  }};
  display: flex;
  align-items: center;
...
export const VaultStatus: React.FC = () => {
  ...
  let content = "The vault is locked. NAPT files cannot be decrypted until you authenticate.";

  if (cryptoCorrupted) {
    status = "error";
    Icon = ShieldAlert;
    label = "Decryption Corrupted";
    content = "The decryption stream has been corrupted. This usually indicates an invalid session key or mismatched environment variables (.env.local).";
  } else if (isUnlocked) {
    status = "unlocked";
    Icon = ShieldCheck;
    label = "Vault Unlocked";
    content = "The vault is unlocked. NAPT files will be decrypted automatically using your session key.";
  } else if (isFailed) {
    status = "error";
    Icon = ShieldAlert;
    label = "Vault Error";
    ...
    <Container>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
        ...
        <span style={{ 
          fontSize: '10px', 
          fontWeight: 'bold', 
          color: status === 'unlocked' ? '#4caf50' : status === 'error' ? '#f44336' : '#888',
