...
const FFTStitcherCanvas: React.FC<FFTStitcherCanvasProps> = ({
  ...
  const buildCombinedFrame = useCallback(
    async (frame: number) => {
      try {
    ...
    try {
      ...
      const result = await fileWorkerManager.stitchFiles(
        ...
        (progress) => {
          ...
          setStitchStatus(
            progress.status || `Loading ${progress.current}/${progress.total} files...`,
          );
        },
      );
  ...
  useEffect(() => {
    fftCanvasDataRef.current = fftCanvasData;
  }, [fftCanvasData]);
