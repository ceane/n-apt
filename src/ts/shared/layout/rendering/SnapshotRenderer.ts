...
/**
 * Trace drawing runs every frame while a snapshot video records, so the
 * intermediate per-pixel-column buffers are reused instead of reallocated.
 * Each borrowed view is consumed synchronously before the next borrow of the
 * same key.
 */
...
function borrowScratch(key: string, size: number): Float32Array {
  ...
  const grown = new Float32Array(Math.max(size, 1024));
  scratchBuffers.set(key, grown);
  return grown.subarray(0, size);
}
...
export class SnapshotRenderer {
  ...
  private drawTraceSteps(
    ...
    const xs = borrowScratch("stepX", dataWidth + 1);
    const ys = borrowScratch("stepY", dataWidth);
    for (let i = 0; i < dataWidth; i++) {
      xs[i] = area.x + i * binW;
      ys[i] = this.mapper.clampY(waveform[i]);
    }
    xs[dataWidth] = rightEdge;
    ...
    dc.beginPath();
    dc.moveTo(area.x, baseline);
    if (crispTrace) {
      for (let i = 0; i < dataWidth; i++) {
        const y = this.mapper.snap(ys[i]);
        dc.lineTo(this.mapper.snap(xs[i]), y);
        dc.lineTo(this.mapper.snap(xs[i + 1]), y);
      }
    } else {
      for (let i = 0; i < dataWidth; i++) {
        const y = ys[i];
    ...
    if (crispTrace) {
      dc.moveTo(
        this.mapper.snapStrokeCenter(area.x),
        this.mapper.snapStrokeCenter(ys[0]),
      );
      for (let i = 0; i < dataWidth; i++) {
        const y = this.mapper.snapStrokeCenter(ys[i]);
        dc.lineTo(this.mapper.snapStrokeCenter(xs[i]), y);
        dc.lineTo(this.mapper.snapStrokeCenter(xs[i + 1]), y);
      }
    } else {
      dc.moveTo(area.x, ys[0]);
      for (let i = 0; i < dataWidth; i++) {
        const y = ys[i];
        dc.lineTo(xs[i], y);
        dc.lineTo(xs[i + 1], y);
  ...
  private drawTraceSmooth(
    ...
    const baseline = area.y + area.height;
    // Frequency is linear in the sample index and freqToX is affine, so the
    // per-sample X positions are an arithmetic sequence.
    const xMin = this.mapper.freqToX(dataRange.min);
    const xMax = this.mapper.freqToX(dataRange.max);
    const xStep = (xMax - xMin) / (dataWidth - 1);

    const xs = borrowScratch("smoothX", dataWidth);
    const ys = borrowScratch("smoothY", dataWidth);
    for (let i = 0; i < dataWidth; i++) {
      xs[i] = xMin + i * xStep;
      ys[i] = this.mapper.clampY(waveform[i]);
    }
    ...
    dc.setFill(this.theme.shadow);
    dc.beginPath();
    dc.moveTo(crispTrace ? this.mapper.snap(xMin) : xMin, baseline);
    if (crispTrace) {
      for (let i = 0; i < dataWidth; i++) {
        dc.lineTo(this.mapper.snap(xs[i]), this.mapper.snap(ys[i]));
      }
    } else {
      for (let i = 0; i < dataWidth; i++) {
        dc.lineTo(xs[i], ys[i]);
    ...
    dc.lineTo(crispTrace ? this.mapper.snap(xMax) : xMax, baseline);
    dc.closePath();
    dc.fill();
    ...
    if (crispTrace) {
      dc.moveTo(
        this.mapper.snapStrokeCenter(xs[0]),
        this.mapper.snapStrokeCenter(ys[0]),
      );
      for (let i = 1; i < dataWidth; i++) {
        dc.lineTo(
          this.mapper.snapStrokeCenter(xs[i]),
          this.mapper.snapStrokeCenter(ys[i]),
        );
      }
    } else {
      dc.moveTo(xs[0], ys[0]);
  ...
  drawStatsBox(
    ...
    let bestFallbackSingle:
      (StatsBoxCandidate & { overlapRatio: number }) | null = null;
