...
import {
  fmtFreqTick,
  tickPrecisionForStep,
} from "@n-apt/layout/rendering/formatters";
import {
  resolveVfoOptions,
  type VfoOptions,
...
export function drawVfoAxis({
  ...
  const options = resolveVfoOptions({
    drawingType: "canvas",
    ...vfoOptions,
  });
  const effectiveTargetTicks =
    options.tickPrecision === "reduced" ? Math.min(targetTicks, 5) : targetTicks;
  const stepHz =
    tickStepHz ?? findBestFrequencyRange(bandwidth, effectiveTargetTicks);
