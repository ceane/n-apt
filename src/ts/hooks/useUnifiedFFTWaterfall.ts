...
export function useUnifiedFFTWaterfall(options: UnifiedFFTWaterfallOptions) {
  ...
  const initializeBuffers = useCallback(() => {
    if (!device) {
      console.warn(
        "GPU device not available for unified buffer initialization",
      );
      return;
    }
    ...
    async (
      ...
      try {
        const expectedInputLength =
          inputMode === "complex_iq" ? fftSize * 2 : fftSize;

        if (inputData.length !== expectedInputLength) {
          throw new Error(
            `Input size mismatch: expected ${expectedInputLength}, got ${inputData.length}`,
          );
        }
        ...
        if (
          ...
          rtlIqWindowPipelineRef.current &&
          bindGroupsRef.current.rtlIqWindow
        ) {
          ...
          fftWindowPipelineRef.current &&
          bindGroupsRef.current.fftWindow
        ) {
        ...
        if (
          enableAveraging &&
          averagingPipelineRef.current &&
          bindGroupsRef.current.averaging
        ) {
          updateParamsWithVals(0, 1, undefined, minDb, maxDb);
        ...
        if (
          enableSmoothing &&
          smoothingPipelineRef.current &&
          bindGroupsRef.current.smoothing
        ) {
          updateParamsWithVals(0, 1, undefined, minDb, maxDb);
        ...
        if (
          waterfallDirectPipelineRef.current &&
          bindGroupsRef.current.waterfall
        ) {
        ...
        if (
          ...
          !spectrumData.some((v) => Number.isFinite(v))
        ) {
