...
import type {
  ...
  PretextLayoutResult,
  PretextMetrics,
} from "@n-apt/components/pretext/PretextTypes";
...
export const usePretextText = (
  options: PretextTextOptions,
): UsePretextTextResult => {
  ...
  const layoutResult = useCallback(
  ...
  const layoutWithLinesResult = useCallback(
    (maxWidth: number, lineHeight?: number) => {
      if (!prepared) {
  ...
  const getDPIScaledMetrics = useCallback(
