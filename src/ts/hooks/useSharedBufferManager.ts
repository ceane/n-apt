...
export function useSharedBufferManager(options: SharedBufferManagerOptions) {
  ...
  const createBuffer = useCallback(
    (size: number, usage: GPUBufferUsageFlags): GPUBuffer => {
      if (!device) {
  ...
  const getBufferFromPool = useCallback(
    (size: number, usage: GPUBufferUsageFlags): GPUBuffer => {
  ...
  const writeToRingBuffer = useCallback(
    (ringBufferId: string, data: ArrayBuffer): boolean => {
      if (!device) {
  ...
  const createSharedComputeBuffer = useCallback(
    (id: string, size: number, owner: string): BufferAllocation => {
      const usage =
  ...
  const updateMemoryStats = useCallback(() => {
    const totalAllocated = Array.from(allocationsRef.current.values()).reduce(
      (sum, alloc) => sum + alloc.size,
      0,
    );

    const ringBufferTotal = Array.from(ringBuffersRef.current.values()).reduce(
      (sum, rb) => sum + rb.capacity * rb.stride,
      0,
    );
  ...
  return {
    ...
    updateMemoryStats,
    getAllocation: useCallback(
      (id: string) => allocationsRef.current.get(id),
      [],
    ),
    getRingBuffer: useCallback(
      (id: string) => ringBuffersRef.current.get(id),
      [],
    ),
    releaseAllocation: useCallback(
      (id: string) => {
        const allocation = allocationsRef.current.get(id);
        if (allocation) {
          returnBufferToPool(allocation.buffer);
