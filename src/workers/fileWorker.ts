...
function loadC64File(fileData, fileName) {
...
function processToSpectrum(rawData, frame = 0, gain = 0) {
  const fftSize = 1024
...
function buildCombinedFrame(fileDataCache, freqMap, frame) {
  const fftSize = 1024
  ...
  try {
    switch (type) {
      ...
      case 'buildFrame': {
        ...
        const cacheMap = new Map(fileDataCache)
        const freqMapObj = new Map(freqMap)
