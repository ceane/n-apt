...
mod tests {
    ...
    fn test_hmac_roundtrip() {
        ...
        assert!(verify_hmac(&key, data, &tag), "HMAC must verify with correct key and data");
    }
    ...
    fn test_hmac_wrong_key_fails() {
        ...
        assert!(!verify_hmac(&key2, data, &tag), "HMAC must fail with wrong key");
    }
    ...
    fn test_hmac_wrong_data_fails() {
        ...
        assert!(!verify_hmac(&key, b"tampered", &tag), "HMAC must fail with wrong data");
    }
    ...
    fn test_hmac_truncated_tag_fails() {
        ...
        assert!(!verify_hmac(&key, b"data", &tag[..16]), "Truncated tag must fail");
    }
    ...
    fn test_encrypt_payload_produces_valid_base64() {
        ...
        assert!(decoded.len() >= 12 + 16, "Encrypted output too short: {} bytes", decoded.len());
    }
    ...
    fn test_encrypt_payload_different_each_time() {
        ...
        assert_ne!(e1, e2, "Encryption must use random IV, producing different ciphertext");
    }
