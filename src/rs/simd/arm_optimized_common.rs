...
impl ARMOptimizedSIMD {
    ...
    pub fn compute_zoomed_data(
        ...
        let visual_center_bin = ((visual_center - full_range_min) / full_span * total_bins).round() as usize;
        
        let mut start_bin = (visual_center_bin as f32 - visible_bins as f32 / 2.0).round() as usize;
        ...
        if zoom < 1.0 {
            ...
            let dest_offset = if start_bin >= usize::MAX { 0 } else { 0 };
            let data_to_copy = (full_waveform.len() as isize).min(visible_bins as isize - dest_offset as isize);
        ...
        start_bin = start_bin.clamp(0, full_waveform.len().saturating_sub(visible_bins));
    ...
    pub fn transform_coordinates_arm_optimized(
        ...
        #[cfg(any(target_arch = "wasm32", target_arch = "aarch64"))]
        {
            Self::transform_coords_arm_impl(
                spectrum_data, canvas_width, canvas_height,
                fft_area_x, fft_area_y, db_min, db_max
            )
        }
    ...
    fn resample_arm_impl(input: &[f32], output: &mut [f32], width: usize) {
        ...
        for i in 0..width {
            ...
            let mut max_vec = f32x4(-f32::INFINITY, -f32::INFINITY, -f32::INFINITY, -f32::INFINITY);
            let mut k = start_idx;
    ...
    fn resample_arm_impl(input: &[f32], output: &mut [f32], width: usize) {
        ...
        unsafe {
            for i in 0..width {
                ...
                let mut max_vec = vdupq_n_f32(-f32::INFINITY);
                let mut k = start_idx;
                
                while k + 4 <= end_idx {
                    let samples = vld1q_f32(input.as_ptr().add(k));
    ...
    fn shift_waterfall_arm_impl(buffer: &mut [u8], width: usize, height: usize) {
        ...
        let cache_lines_per_row = (row_bytes + CACHE_LINE_SIZE - 1) / CACHE_LINE_SIZE;
        ...
        for y in (1..height).rev() {
            ...
            for cache_line in 0..cache_lines_per_row {
                ...
                if line_start < row_bytes && line_end <= row_bytes && line_start + 64 <= buffer.len() {
                    // Load 16 pixels (64 bytes) into 4 SIMD registers
                    unsafe {
                        let src_ptr = buffer.as_ptr().add(src_row_start + line_start) as *const v128;
                        let dst_ptr = buffer.as_mut_ptr().add(dst_row_start + line_start) as *mut v128;
    ...
    fn shift_waterfall_arm_impl(buffer: &mut [u8], width: usize, height: usize) {
        ...
        for y in (1..height).rev() {
            ...
            if src_row_start + row_bytes <= buffer.len() && dst_row_start + row_bytes <= buffer.len() {
    ...
    #[allow(dead_code)]
    fn shift_waterfall_scalar_impl(buffer: &mut [u8], width: usize, height: usize) {
        let row_bytes = width * 4;
        for y in (1..height).rev() {
            let src_row_start = ((y - 1) * width) * 4;
            let dst_row_start = (y * width) * 4;
    ...
    #[cfg(target_arch = "wasm32")]
    fn color_map_arm_impl(amplitudes: &[f32], output: &mut [u8], color_intensity: f32) {
        use std::arch::wasm32::*;
        
        let intensity_vec = f32x4(color_intensity, color_intensity, color_intensity, color_intensity);
        ...
        while i + 4 <= amplitudes.len() {
            let amp_vec = unsafe { v128_load(amplitudes.as_ptr().add(i) as *const v128) };
        ...
        while i < amplitudes.len() {
            let val = (amplitudes[i] * color_intensity * 255.0).clamp(0.0, 255.0) as u8;
            let idx = i * 4;
            if idx + 4 <= output.len() {
                output[idx] = val;
                output[idx + 1] = val;
    ...
    #[allow(dead_code)]
    fn color_map_scalar_impl(amplitudes: &[f32], output: &mut [u8], color_intensity: f32) {
    ...
    fn transform_coords_arm_impl(
        ...
        let x_scale = if data_width > 1 { plot_width / (data_width as f32 - 1.0) } else { 0.0 };
    ...
    fn transform_coords_arm_impl(
        ...
        for (i, &db_val) in spectrum_data.iter().enumerate() {
            let x = x_base + (i as f32) * x_scale;
            let y = (fft_area_max_y - (db_val - db_min) * scale_factor).clamp(fft_area_y as f32 + 1.0, fft_area_max_y);
            coords.push((x, y));
        }
    ...
    fn transform_coords_scalar_impl(
        ...
        for (i, &value) in spectrum_data.iter().enumerate() {
            let x = fft_area_x as f32 + (i as f32 / (data_width as f32 - 1.0)) * plot_width;
...
pub trait UnifiedSIMDOps {
    ...
    fn shift_waterfall_buffer(&self, buffer: &mut [u8], width: usize, height: usize);
    fn apply_color_mapping(&self, amplitudes: &[f32], output: &mut [u8], color_intensity: f32);
}
...
mod tests {
    ...
    fn test_zoomed_data_computation() {
        ...
        let (sliced, min, max, pan) = ARMOptimizedSIMD::compute_zoomed_data(
            &waveform, 100.0, 200.0, 2.0, 0.0
        );
...
impl ARMOptimizedSIMD {
    ...
    pub fn apply_window_arm_optimized(
        ...
        let len = complex_re.len().min(complex_im.len()).min(window_coeffs.len());
        if len == 0 { return; }
        ...
        #[cfg(not(any(target_arch = "wasm32", target_arch = "aarch64")))]
        {
            Self::apply_window_scalar_impl(complex_re, complex_im, window_coeffs, len);
        }
    }
    ...
    pub fn to_power_spectrum_db_arm_optimized(
        ...
        #[cfg(any(target_arch = "wasm32", target_arch = "aarch64"))]
        {
            Self::power_spectrum_db_arm_impl(complex_re, complex_im, output, len, inv_norm);
        }

        #[cfg(not(any(target_arch = "wasm32", target_arch = "aarch64")))]
        {
            Self::power_spectrum_db_scalar_impl(complex_re, complex_im, output, len, inv_norm);
        }
    }
    ...
    #[cfg(target_arch = "aarch64")]
    fn apply_window_arm_impl(complex_re: &mut [f32], complex_im: &mut [f32], window_coeffs: &[f32], len: usize) {
        use std::arch::aarch64::*;
        unsafe {
            let mut i = 0;
            while i + 4 <= len {
    ...
    fn apply_window_arm_impl(complex_re: &mut [f32], complex_im: &mut [f32], window_coeffs: &[f32], len: usize) {
        ...
        unsafe {
            ...
            while i + 4 <= len {
                ...
                v128_store(complex_re.as_mut_ptr().add(i) as *mut v128, f32x4_mul(re, w));
                v128_store(complex_im.as_mut_ptr().add(i) as *mut v128, f32x4_mul(im, w));
                i += 4;
            }
            while i < len {
                complex_re[i] *= window_coeffs[i];
    ...
    #[allow(dead_code)]
    fn apply_window_scalar_impl(complex_re: &mut [f32], complex_im: &mut [f32], window_coeffs: &[f32], len: usize) {
        for i in 0..len {
            complex_re[i] *= window_coeffs[i];
            complex_im[i] *= window_coeffs[i];
        }
    ...
    #[cfg(target_arch = "aarch64")]
    fn power_spectrum_db_arm_impl(complex_re: &[f32], complex_im: &[f32], output: &mut [f32], len: usize, inv_norm: f32) {
        use std::arch::aarch64::*;
        ...
        unsafe {
            ...
            while i < len {
                let mag_sq = (complex_re[i] * complex_re[i] + complex_im[i] * complex_im[i]) * inv_norm;
    ...
    #[allow(dead_code)]
    fn power_spectrum_db_scalar_impl(complex_re: &[f32], complex_im: &[f32], output: &mut [f32], len: usize, inv_norm: f32) {
        for i in 0..len {
            let mag_sq = (complex_re[i] * complex_re[i] + complex_im[i] * complex_im[i]) * inv_norm;
            output[i] = 10.0 * (mag_sq + 1e-12).log10().max(-120.0).min(0.0);
        }
    ...
    fn convert_to_complex_arm_impl(
        ...
        unsafe {
            let v_gain = f32x4(gain / 128.0, gain / 128.0, gain / 128.0, gain / 128.0);
            ...
            while i + 4 <= len {
                ...
                let f_high = f32x4_mul(f32x4_sub(f32x4(b4, b5, b6, b7), v_bias), v_gain);
