...
/// # Safety
/// This function uses raw NEON intrinsics and assumes the input is a valid f32x4 vector.
#[cfg(target_arch = "aarch64")]
#[inline(always)]
pub unsafe fn fast_expq_f32(mut x: float32x4_t) -> float32x4_t {
  // x * log2(e)
  x = vmulq_f32(x, vdupq_n_f32(std::f32::consts::LOG2_E));
  ...
  // 2^frac via polynomial: 1.0 + r*ln(2) + r^2*0.2402265 + r^3*0.0558011
  // Computed via Horner's method: 1.0 + r * (ln(2) + r * (0.2402265 + r * 0.0558011))
  let mut p = vfmaq_f32(vdupq_n_f32(0.2402265), vdupq_n_f32(0.0558011), r);
  p = vfmaq_f32(vdupq_n_f32(std::f32::consts::LN_2), p, r);
...
/// # Safety
/// This function uses raw WASM SIMD intrinsics and assumes the input is a valid f32x4 vector.
#[cfg(target_arch = "wasm32")]
#[inline(always)]
pub unsafe fn fast_expq_f32(mut x: v128) -> v128 {
  // x * log2(e)
  let log2e = std::f32::consts::LOG2_E;
  x = f32x4_mul(x, f32x4(log2e, log2e, log2e, log2e));
...
///
/// # Safety
/// This function uses raw NEON intrinsics and calls unsafe fast_expq_f32.
#[cfg(target_arch = "aarch64")]
#[inline(always)]
pub unsafe fn fast_pow10q_f32(x: float32x4_t) -> float32x4_t {
  // ln(10)
  let x_ln10 = vmulq_f32(x, vdupq_n_f32(std::f32::consts::LN_10));
  fast_expq_f32(x_ln10)
}

/// # Safety
/// This function uses raw WASM SIMD intrinsics and calls unsafe fast_expq_f32.
#[cfg(target_arch = "wasm32")]
#[inline(always)]
pub unsafe fn fast_pow10q_f32(x: v128) -> v128 {
  let ln10 = std::f32::consts::LN_10;
  let x_ln10 = f32x4_mul(x, f32x4(ln10, ln10, ln10, ln10));
  fast_expq_f32(x_ln10)
}
...
pub unsafe fn fast_log10q_f32(x: float32x4_t) -> float32x4_t {
  ...
  p = vfmaq_f32(vdupq_n_f32(1.860_639), p, m);

  let log2_val = vaddq_f32(vcvtq_f32_s32(e), p);

  // log10(x) = log2(x) * log10(2)
  // log10(2) = 0.301029995
  vmulq_f32(log2_val, vdupq_n_f32(0.301_029_98))
}
...
pub unsafe fn fast_log10q_f32(x: v128) -> v128 {
  ...
  f32x4_mul(
    log2_val,
    f32x4(0.301029995, 0.301029995, 0.301029995, 0.301029995),
  )
}
...
pub unsafe fn fast_sinq_f32(mut x: float32x4_t) -> float32x4_t {
  let inv_2pi = vdupq_n_f32(0.159_154_94);
  ...
  let mut p =
    vfmaq_f32(vdupq_n_f32(0.008333333), vdupq_n_f32(-0.000198412), x2); // 1/120, -1/5040
  p = vfmaq_f32(vdupq_n_f32(-0.166_666_67), p, x2); // -1/6
...
pub unsafe fn fast_sinq_f32(mut x: v128) -> v128 {
  let inv_2pi =
    f32x4(0.15915494309, 0.15915494309, 0.15915494309, 0.15915494309);
  let two_pi = f32x4(
...
///
/// # Safety
/// This function uses raw NEON intrinsics and calls unsafe fast_sinq_f32.
#[cfg(target_arch = "aarch64")]
#[inline(always)]
...
/// # Safety
/// This function uses raw WASM SIMD intrinsics and calls unsafe fast_sinq_f32.
#[cfg(target_arch = "wasm32")]
#[inline(always)]
