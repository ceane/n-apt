...
impl MockSignalGenerator {
  ...
  pub fn generate_frame(
    ...
    {
        self.generate_frame_simd(&mut frame, fft_size, sample_rate, signals, noise_level, t0);
    }
    ...
    #[cfg(not(any(target_arch = "wasm32", target_arch = "aarch64")))]
    {
        for i in 0..fft_size {
          ...
          let (i_sample, q_sample) = self.generate_sample(i, t, signals, noise_level, sample_rate, fft_size);

          // Convert to u8 range
          frame[i * 2] = ((i_sample * 127.0) + 128.0).round().clamp(0.0, 255.0) as u8;
          frame[i * 2 + 1] = ((q_sample * 127.0) + 128.0).round().clamp(0.0, 255.0) as u8;
        }
    }
  ...
  fn generate_frame_simd(
    ...
    t0: f32
      ...
      for s in signals {
          ...
          let pps = 2.0 * std::f64::consts::PI * freq_hz as f64 / sample_rate as f64;
          ...
          if s.bandwidth > 1 {
              ...
              if side > 0 {
                  for mult in [-side, side] {
                      ...
                      let pps2 = 2.0 * std::f64::consts::PI * freq2 as f64 / sample_rate as f64;
                      let (s_re, s_im, s_rot_re, s_rot_im) = build_phasor(pps2);
                      side_tones.push(ActiveSideTone {
                          amp_weight: 0.5,
                          v_re: s_re, v_im: s_im,
                          rot_re: s_rot_re, rot_im: s_rot_im,
                      });
                  }
              }
          }
          active_list.push(ActiveSignal { amp, v_re, v_im, rot_re, rot_im, side_tones });
      }
      ...
      unsafe {
          ...
          for i in (0..fft_size).step_by(4) {
              if i + 4 > fft_size { break; } 

              #[cfg(target_arch = "aarch64")]
              let (mut i_acc, mut q_acc) = {
                  let noise_i = vdupq_n_f32((self.rng.gen::<f32>() - 0.5) * 2.0 * noise_level);
                  let noise_q = vdupq_n_f32((self.rng.gen::<f32>() - 0.5) * 2.0 * noise_level);
                  (noise_i, noise_q)
              };
              #[cfg(target_arch = "wasm32")]
              let (mut i_acc, mut q_acc) = {
                  ...
                  (f32x4(n_val_i, n_val_i, n_val_i, n_val_i), f32x4(n_val_q, n_val_q, n_val_q, n_val_q))
              };
              ...
              for sig in &mut active_list {
                  ...
                  #[cfg(target_arch = "aarch64")] {
                      ...
                      let next_re = vsubq_f32(vmulq_f32(s_re, v_rot_re), vmulq_f32(s_im, v_rot_im));
                      let next_im = vaddq_f32(vmulq_f32(s_im, v_rot_re), vmulq_f32(s_re, v_rot_im));
                      vst1q_f32(sig.v_re.as_mut_ptr(), next_re);
                      vst1q_f32(sig.v_im.as_mut_ptr(), next_im);
                  }
                  #[cfg(target_arch = "wasm32")] {
                      let v_rot_re = f32x4(sig.rot_re, sig.rot_re, sig.rot_re, sig.rot_re);
                      let v_rot_im = f32x4(sig.rot_im, sig.rot_im, sig.rot_im, sig.rot_im);
                      let next_re = f32x4_sub(f32x4_mul(s_re, v_rot_re), f32x4_mul(s_im, v_rot_im));
                      let next_im = f32x4_add(f32x4_mul(s_im, v_rot_re), f32x4_mul(s_re, v_rot_im));
                  ...
                  for side in &mut sig.side_tones {
                      ...
                      #[cfg(target_arch = "wasm32")]
                      let v_side_amp = f32x4(sig.amp * side.amp_weight, sig.amp * side.amp_weight, sig.amp * side.amp_weight, sig.amp * side.amp_weight);
                      ...
                      #[cfg(target_arch = "aarch64")] {
                          ...
                          let next_re = vsubq_f32(vmulq_f32(ss_re, v_rot_re), vmulq_f32(ss_im, v_rot_im));
                          let next_im = vaddq_f32(vmulq_f32(ss_im, v_rot_re), vmulq_f32(ss_re, v_rot_im));
                          vst1q_f32(side.v_re.as_mut_ptr(), next_re);
                          vst1q_f32(side.v_im.as_mut_ptr(), next_im);
                      }
                      #[cfg(target_arch = "wasm32")] {
                          let v_rot_re = f32x4(side.rot_re, side.rot_re, side.rot_re, side.rot_re);
                          let v_rot_im = f32x4(side.rot_im, side.rot_im, side.rot_im, side.rot_im);
                          let next_re = f32x4_sub(f32x4_mul(ss_re, v_rot_re), f32x4_mul(ss_im, v_rot_im));
                          let next_im = f32x4_add(f32x4_mul(ss_im, v_rot_re), f32x4_mul(ss_re, v_rot_im));
              ...
              #[cfg(target_arch = "aarch64")]
              {
                  ...
                  frame[(i + 0) * 2] = vgetq_lane_u32(i_u32, 0) as u8;
                  frame[(i + 0) * 2 + 1] = vgetq_lane_u32(q_u32, 0) as u8;
