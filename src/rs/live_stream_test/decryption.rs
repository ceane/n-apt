...
mod tests {
  ...
  fn test_encrypt_decrypt_roundtrip() {
    ...
    let encrypted =
      crate::crypto::encrypt_payload_binary(&key, plaintext).unwrap();
  ...
  fn test_waveform_decryption() {
    ...
    let original_waveform: Vec<f32> =
      vec![1.0, -1.0, 0.5, -0.5, 0.0, 2.0, -2.0, 1.5];
    let waveform_bytes: &[u8] = bytemuck::cast_slice(&original_waveform);

    let encrypted =
      crate::crypto::encrypt_payload_binary(&key, waveform_bytes).unwrap();
