...
pub async fn load_local_radius_towers(
    ...
    info!("Loading local towers: lat={}, lng={}, radius={}km", 
          request.latitude, request.longitude, radius_km);
    
    // Check if already cached
    if let Ok(cached_result) = check_local_cache(request.latitude, request.longitude, radius_km).await {
        info!("Found cached local towers: {}", cached_result.loaded);
        return Ok(Json(cached_result));
    }
    
    // Load towers dynamically
    match load_towers_command(request.latitude, request.longitude, radius_km).await {
        Ok(result) => {
 ...
 */
async fn check_local_cache(lat: f64, lng: f64, radius_km: u32) -> Result<LoadLocalRadiusResponse, Box<dyn std::error::Error>> {
 ...
 */
async fn load_towers_command(lat: f64, lng: f64, radius_km: u32) -> Result<LoadLocalRadiusResponse, Box<dyn std::error::Error>> {
 ...
 */
pub async fn get_local_cache_stats() -> Result<Json<serde_json::Value>, StatusCode> {
    let redis_url = std::env::var("REDIS_URL").unwrap_or_else(|_| "redis://127.0.0.1/".to_string());
        ...
        Ok(client) => {
            match client.get_connection() {
                Ok(mut con) => {
                    ...
                    if redis::cmd("SELECT").arg(4).query::<String>(&mut con).is_err() {
                        return Err(StatusCode::SERVICE_UNAVAILABLE);
                    }
                    
                    // Get memory info
                    let info: String = match redis::cmd("INFO").arg("memory").query(&mut con) {
                        Ok(info) => info,
                        Err(_) => return Err(StatusCode::SERVICE_UNAVAILABLE),
                    };
                    
                    // Count local cache keys
                    let keys: Vec<String> = match redis::cmd("KEYS").arg("local:*").query(&mut con) {
                        Ok(keys) => keys,
                        Err(_) => return Err(StatusCode::SERVICE_UNAVAILABLE),
                    };
...
fn parse_memory_info(info: &str) -> HashMap<String, String> {
    ...
    for line in info.lines() {
        if line.starts_with("used_memory:") {
            ...
            if let Some(value) = line.split(':').nth(1) {
                memory_info.insert("used_memory_human".to_string(), value.trim().to_string());
            }
        }
    }
