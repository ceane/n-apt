...
pub async fn example_websocket_usage() -> Result<()> {
    ...
    secure_handler.process_message(&sample_message, &broadcast_tx).await?;
...
impl EnhancedWebSocketHandler {
    ...
    pub async fn handle_message(
        ...
        if let Err(_) = self.secure_handler.process_message(message, broadcast_tx).await {
            // Fallback to regular processing if secure processing unavailable
            warn!("Secure processing unavailable, using regular handler");
            self.regular_handler.handle_message(message, broadcast_tx).await?;
        }
