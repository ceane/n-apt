...
impl SessionStore {
  ...
  pub fn new() -> Self {
    ...
    if let Ok(data) = std::fs::read_to_string(&persist_path) {
      if let Ok(persisted) =
        serde_json::from_str::<Vec<PersistedSession>>(&data)
      {
        let now = SystemTime::now()
          .duration_since(UNIX_EPOCH)
          .unwrap_or_default()
...
mod tests {
  ...
  fn temp_session_path() -> std::path::PathBuf {
    let dir =
      std::env::temp_dir().join(format!("n-apt-test-{}", Uuid::new_v4()));
  ...
  fn test_expired_session_not_valid() {
    ...
    let store =
      SessionStore::with_path_and_ttl(path.clone(), Duration::from_secs(0));
    let key = [99u8; 32];
  ...
  fn test_session_persists_to_disk() {
    ...
    if let Ok(data) = fs::read_to_string(&path) {
        ...
        serde_json::from_str::<Vec<serde_json::Value>>(&data)
      {
        assert!(
          !persisted.is_empty(),
          "Persisted sessions should not be empty"
        );
        let has_token = persisted
          .iter()
          .any(|p| p.get("token").and_then(|t| t.as_str()) == Some(&token));
