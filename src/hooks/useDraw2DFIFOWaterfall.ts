...
export function useDraw2DFIFOWaterfall() {
  ...
  const dbToColor = useCallback((db: number, minDb: number, maxDb: number): [number, number, number] => {
  ...
  const draw2DFIFOWaterfall = useCallback((options: Draw2DFIFOWaterfallOptions) => {
    const {
      canvas,
      waterfallBuffer,
      frequencyRange,
