...
export function useDraw2DFFTSignal() {
  ...
  const draw2DFFTSignal = useCallback((options: Draw2DFFTSignalOptions) => {
    const {
      canvas,
      waveform,
      frequencyRange,
    ...
    try {
      ...
      if (centerFrequencyMHz !== undefined) {
        drawSpectrumMarkers(ctx, width, height, frequencyRange, centerFrequencyMHz, isDeviceConnected);
      }
