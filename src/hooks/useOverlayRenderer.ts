...
export function useOverlayRenderer() {
  ...
  const drawMarkersOnContext = useCallback((
    ...
    if (centerFrequencyMHz !== undefined && centerFrequencyMHz >= minFreq && centerFrequencyMHz <= maxFreq) {
    ...
    const centerLabel =
      centerFrequencyMHz === undefined || Number.isNaN(centerFrequencyMHz) || !Number.isFinite(centerFrequencyMHz)
        ? "✋  -- MHz"
