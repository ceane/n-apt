...
const SpectrumVisualizer: React.FC<SpectrumVisualizerProps> = ({ 
  ...
  const waterfallCtxRef = useRef<CanvasRenderingContext2D | null>(null)
  ...
  const drawFrame = useCallback(() => {
    ...
    if (!spectrumCanvas || !waterfallCanvas || !spectrumCtx || !waterfallCtx) return
  ...
  useEffect(() => {
    ...
    const resizeCanvases = () => {
      ...
      if (!spectrumWrapper || !waterfallWrapper) return
