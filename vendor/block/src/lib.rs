...
// Local patch: upstream declares this as an empty `enum Class { }`, which makes
// `static _NSConcreteStackBlock: Class` an uninhabited static (deny-by-default
// `uninhabited_static`/`future_incompatible` lint). An opaque zero-sized struct
// keeps the same ABI use (address-only, never dereferenced) without the lint.
#[repr(C)]
struct Class {
    _private: [usize; 0],
}
