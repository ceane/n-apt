...
fn fft_benchmark(c: &mut Criterion) {
    ...
    for size in [2048, 4096, 8192, 16384, 32768, 65536, 131072, 262144].iter() {
        let size = *size;
        let mut processor = FFTProcessor::new_with_defaults(size, sample_rate, min_db, max_db);
        ...
        group.bench_with_input(format!("Size {}", size), &samples, |b, s| {
            b.iter(|| {
                processor.process_samples(black_box(s)).unwrap()
            });
        });
    }
    group.finish();
...
fn window_benchmark(c: &mut Criterion) {
    ...
    for window in [WindowType::Rectangular, WindowType::Hanning, WindowType::Hamming, WindowType::Blackman].iter() {
        let mut processor = FFTProcessor::new_with_defaults(size, sample_rate, min_db, max_db);
