...
describe("Backend-Frontend Hz Pipeline Integration (True E2E)", () => {
  ...
  beforeAll(() => {
    ...
    try {
      ...
      console.error(
        "Failed to run Rust dump_config binary. Ensure cargo is installed and working.",
      );
      throw error;
    }
  });
  ...
  test("should validate real backend-processed Hz values from signals.yaml", () => {
    ...
    expect(channel.min_hz).toBe(
      signalsConfig.signals.n_apt.channels[channel.id].freq_range_hz[0],
    );
  });
  ...
  test("should correctly map real backend config to Redux state", () => {
    ...
    const mappedFrame = action.payload.spectrumFrames!.find(
      (f: any) => f.id === firstId,
    );
