...
jest.mock("katex", () => ({
  ...
  default: {
    renderToString: jest.fn(
      (expression: string) => `<span>${expression}</span>`,
    ),
  },
}));
jest.mock("react-markdown", () => ({
  ...
  default: ({ children }: any) => {
    ...
    markdown.split("\n").forEach((line, index) => {
      if (line.startsWith("## ")) {
        elements.push(
          React.createElement("h2", { key: `h2-${index}` }, line.slice(3)),
        );
      }
    });
    ...
    if (markdown.includes("canvas::bodyattenuation")) {
      elements.push(
        React.createElement("body-attenuation-canvas", { key: "body" }),
      );
    }
    if (markdown.includes("canvas::timeofflight")) {
      elements.push(
        React.createElement("time-of-flight-canvas", { key: "tof" }),
      );
    }
    if (markdown.includes("canvas::impedance")) {
      elements.push(
        React.createElement("impedance-canvas", { key: "impedance" }),
      );
    }
    if (markdown.includes("canvas::phaseshifting")) {
      elements.push(
        React.createElement("phase-shifting-canvas", { key: "phase" }),
      );
    }
    if (markdown.includes("canvas::amplitudemodulation")) {
      elements.push(
        React.createElement("amplitude-modulation-canvas", { key: "am" }),
      );
    }
    ...
    if (markdown.includes("```latex")) {
      ...
      const expressions = matches
        ...
        .filter(Boolean);
      elements.push(
        React.createElement("latex-block", {
          key: "latex",
...
describe("Markdown Preview System", () => {
  ...
  test("fetches the default markdown source with no-cache headers", async () => {
    ...
    await waitFor(() => {
      expect(fetch).toHaveBeenCalledWith("/pages/how-do-they-do-it.md", {
...
\`\`\`
    ...
    await waitFor(() => {
      expect(
        document.querySelector("body-attenuation-canvas"),
      ).toBeInTheDocument();
      expect(
        document.querySelector("time-of-flight-canvas"),
      ).toBeInTheDocument();
      expect(document.querySelector("impedance-canvas")).toBeInTheDocument();
      expect(
        document.querySelector("phase-shifting-canvas"),
      ).toBeInTheDocument();
      expect(
        document.querySelector("amplitude-modulation-canvas"),
      ).toBeInTheDocument();
    });
  });
...
\`\`\`
    ...
    await waitFor(() => {
      ...
      const expressions = JSON.parse(
        decodeURIComponent(
          latexBlock?.getAttribute("data-expressions") ?? "[]",
        ),
      );
