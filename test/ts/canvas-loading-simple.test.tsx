...
jest.mock("three", () => ({
  ...
  DirectionalLight: jest
    .fn()
    .mockImplementation(() => ({ color: "", intensity: 0 })),
  AmbientLight: jest
    .fn()
    .mockImplementation(() => ({ color: "", intensity: 0 })),
  ...
  MeshBasicMaterial: jest
    .fn()
    .mockImplementation(() => ({ material: "basic" })),
  MeshStandardMaterial: jest
    .fn()
    .mockImplementation(() => ({ material: "standard" })),
  Mesh: jest.fn().mockImplementation(() => ({ mesh: "test" })),
  BufferGeometry: jest.fn().mockImplementation(() => ({ geometry: "buffer" })),
  BufferAttribute: jest
    .fn()
    .mockImplementation(() => ({ attribute: "buffer" })),
  InterleavedBufferAttribute: jest
    .fn()
    .mockImplementation(() => ({ attribute: "interleaved" })),
...
describe("Canvas Component Loading Tests (Simple)", () => {
  ...
  describe("Canvas File Structure", () => {
    test("should have all canvas component files", async () => {
      ...
      const componentsDir = path.join(
        __dirname,
        "../../src/md-preview/components/canvas",
      );
      const canvasFiles = [
        "index.ts",
        "helpers.tsx",
  ...
  describe("Canvas Dependencies", () => {
    test("should import required dependencies", async () => {
      ...
      const amplitudeModulationPath = path.join(
        componentsDir,
        "AmplitudeModulationCanvas.tsx",
      );
      const amplitudeModulationContent = fs.readFileSync(
        amplitudeModulationPath,
        "utf8",
      );
