...
const createMockTree = (markdown: string, explicitLang?: string): any => ({
  ...
  children: [
    ...
    },
    {
      ...
      lang:
        explicitLang ??
        (markdown.includes("canvas::bodyattenuation")
          ? "canvas::bodyattenuation"
          : markdown.includes("canvas::timeofflight")
            ? "canvas::timeofflight"
            : markdown.includes("canvas::phaseshifting")
              ? "canvas::phaseshifting"
              : markdown.includes("canvas::frequencymodulation")
                ? "canvas::frequencymodulation"
                : markdown.includes("canvas::amplitudemodulation")
                  ? "canvas::amplitudemodulation"
                  : markdown.includes("canvas::heterodyning")
                    ? "canvas::heterodyning"
                    : markdown.includes("canvas::multipath")
                      ? "canvas::multipath"
                      : markdown.includes("latex")
                        ? "latex"
                        : markdown.includes("tex")
                          ? "tex"
                          : markdown.startsWith("canvas::")
                            ? markdown
                            : undefined),
      value: markdown,
    },
  ...
  describe("Signal Canvas Plugin", () => {
    test("should identify and replace signal canvas blocks", () => {
      ...
      const visitNode = (node: any, index: number, parent: any) => {
        if (
          node.type === "code" &&
          node.lang === "canvas::amplitudemodulation"
        ) {
          const replacement = {
            type: "html",
            value:
  ...
  describe("LaTeX Plugin", () => {
    test("should process LaTeX expressions correctly", () => {
      ...
      const visitNode = (node: any, index: number, parent: any) => {
        if (
          node.type === "code" &&
          (node.lang === "latex" || node.lang === "tex")
        ) {
          const expressions = ["E = mc^2"];
          const serializedExpressions = encodeURIComponent(
    ...
    test("should handle multiple LaTeX expressions", () => {
      ...
      const decoded = JSON.parse(
        decodeURIComponent(
          result.value.match(/data-expressions="([^"]*)"/)![1],
        ),
      );
  ...
  describe("Icon Shortcodes Plugin", () => {
    test("should replace icon shortcodes with HTML elements", () => {
      ...
      expect(tree.children[0].children[1].value).toBe(
        '<icon-inline data-icon="heart"></icon-inline>',
      );
      expect(tree.children[0].children[2].value).toBe(" and another: ");
      expect(tree.children[0].children[3].value).toBe(
        '<icon-inline data-icon="star"></icon-inline>',
      );
    });
