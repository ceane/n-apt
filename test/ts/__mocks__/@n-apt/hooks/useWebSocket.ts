...
export function useWebSocket(url: string | null, aesKey: CryptoKey | null, enabled: boolean): WebSocketData {
  ...
  const sendTrainingCommand = useCallback((action: "start" | "stop", label: "target" | "noise", signalArea: string) => {
    if (!isConnected) {
      setError("Not connected")
      return
    }
