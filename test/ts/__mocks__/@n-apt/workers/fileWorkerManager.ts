export class FileWorkerManager {
  ...
  async buildFrame(frame: number, fileDataCache: Map<string, Uint8Array>, freqMap: Map<string, number>): Promise<any> {
    if (this.isTerminated) {
      throw new Error("Worker terminated")
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      }, 50 + Math.random() * 100) // Variable processing time
    })
  }
  ...
  simulateError(errorType: 'timeout' | 'crash' | 'memory') {
    switch (errorType) {
      case 'timeout':
