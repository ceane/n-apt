...
const MockFFTCanvas = React.forwardRef<HTMLDivElement, any>(
  ({ ...props }, ref) => {
...
const MockFFTPlaybackCanvas = React.forwardRef<HTMLDivElement, any>(
  ({ ...props }, ref) => {
    // Filter out component-specific props that shouldn't be passed to DOM elements
...
const MockFIFOWaterfall: React.FC<any> = ({
  width,
  height,
  waveform,
  ...props
  ...
  return (
    <div data-testid="fifo-waterfall" {...domProps}>
      <canvas data-testid="waterfall-canvas" width={width} height={height} />
      {waveform ? (
        <div data-testid="waterfall-data">Data loaded</div>
      ) : (
        <div data-testid="waterfall-placeholder">
...
describe("WebApp Canvas Components", () => {
  ...
  describe("FFTPlaybackCanvas", () => {
    ...
    test("displays channel selector for multi-channel files", () => {
      ...
      expect(screen.getByTestId("channel-selector")).toHaveTextContent(
        "Channel: 1/2",
      );
    });
  ...
  describe("FIFOWaterfall", () => {
    ...
    test("renders without waveform data", () => {
      ...
      expect(screen.getByTestId("waterfall-placeholder")).toHaveTextContent(
        "Loading data from source...",
      );
    });
    ...
    test("renders with waveform data", () => {
      ...
      expect(screen.getByTestId("waterfall-data")).toHaveTextContent(
        "Data loaded",
      );
    });
