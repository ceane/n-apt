...
describe("Markdown Preview Assets", () => {
  ...
  const CANVAS_SOURCE_OVERRIDES: Record<string, string> = {
    ...
    "BodyAttenuationWebGPUCanvas.tsx":
      "components/canvas/BodyAttenuationCanvas.tsx",
  ...
  function resolveCanvasSourcePath(
  ...
  describe("Remark Plugin Structure", () => {
    ...
    test("should have correct canvas language identifiers", () => {
      ...
      expect(signalCanvasContent).toMatch(
        /canvas::(phaseshifting|frequencymodulation|amplitudemodulation)/,
      );
    });
  ...
  describe("Canvas Component Structure", () => {
    test("should have properly structured canvas components", () => {
      ...
      canvasComponents.forEach((componentFile) => {
        const componentPath = resolveCanvasSourcePath(
          componentsDir,
          componentFile,
        );
