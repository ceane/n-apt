...
fn test_process_dc_signal() {
  ...
  assert!(
    dc_power > avg_non_dc,
    "DC bin (at center) should dominate for a DC signal: DC={}, avg={}",
    dc_power,
    avg_non_dc
  );
}
...
fn test_process_samples_center_is_zero() {
  ...
  for &val in result.iter() {
    assert!(
      val.is_finite(),
      "Zero input should produce a finite floor value, got {}",
      val
    );
    assert_eq!(
      val, -150.0,
      "Zero input should clamp to -150 dB, got {}",
      val
    );
  }
}
...
fn test_gain_affects_output() {
  let fft_size = 128;
  let samples: Vec<u8> =
    (0..fft_size * 2).map(|i| (128 + (i % 50)) as u8).collect();
  ...
  assert!(
    avg_high > avg_low,
    "Higher gain should produce stronger output: low={}, high={}",
    avg_low,
    avg_high
  );
}
...
fn test_window_rectangular_vs_hanning() {
  let fft_size = 256;
  let samples: Vec<u8> = (0..fft_size * 2)
    .map(|i| {
  ...
  let differs = result_rect
    .iter()
    .zip(result_hann.iter())
    .any(|(a, b)| (a - b).abs() > 0.01);
  assert!(
    differs,
    "Hanning and Rectangular windows should produce different results"
  );
}
...
fn test_all_window_types() {
  let fft_size = 128;
  let samples: Vec<u8> =
    (0..fft_size * 2).map(|i| (128 + (i % 40)) as u8).collect();
  ...
  for window in &[
    ...
    assert_eq!(
      result.len(),
      fft_size,
      "Window '{}' should produce correct output length",
      window
    );
  }
}
...
fn test_simd_availability() {
  ...
  assert_eq!(
    result.len(),
    4,
    "SIMD processor should produce correct output length"
  );
}
...
fn test_math_functions_in_wasm() {
  ...
  assert!(
    (sin_val - 1.0).abs() < 0.01,
    "sin(pi/2) should be approx 1.0, got {}",
    sin_val
  );
}
...
fn test_fft_performance() {
  ...
  assert!(
    elapsed < 5000.0,
    "10x 1024-point FFT should complete in < 5s, took {}ms",
    elapsed
  );
}
...
fn test_process_samples_large_fft() {
  ...
  assert!(
    elapsed < 1000.0,
    "8192-point FFT should complete in < 1s, took {}ms",
    elapsed
  );
