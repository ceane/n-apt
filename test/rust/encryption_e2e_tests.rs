...
fn test_encryption_save_load_cycle() {
    ...
    // 2. Save file
    let artifact = save_capture_file_multi(&result, &vault_key).expect("Save failed");
    
    // 3. Verify Checksum exists
    assert!(!artifact.checksum.is_empty(), "Checksum should be generated");
    ...
    let header_json: serde_json::Value = serde_json::from_str(header_str.trim()).expect("Header parse failed");
    let wrapped_dek_b64 = header_json["metadata"]["wrapped_dek"].as_str().expect("wrapped_dek missing");
    let wrapped_dek_bytes = crypto::from_base64(wrapped_dek_b64).expect("Base64 decode failed");
    
    // 7. Unwrap DEK using Vault Key
    let dek_bytes = crypto::decrypt_payload_binary(&vault_key, &wrapped_dek_bytes).expect("DEK unwrap failed");
    let mut dek = [0u8; 32];
    dek.copy_from_slice(&dek_bytes);
    
    // 8. Decrypt Payload using DEK
    let decrypted_payload = crypto::decrypt_payload_binary(&dek, payload_bytes).expect("Payload decryption failed");
    ...
    let expected_checksum = hasher.finalize().iter().map(|b| format!("{:02x}", b)).collect::<String>();
    assert_eq!(artifact.checksum, expected_checksum, "Checksum verification failed");
}
...
fn test_checksum_integrity_and_corruption() {
    ...
    let mut hasher = sha2::Sha256::new();
    hasher.update(&file_bytes);
    let new_checksum = hasher.finalize().iter().map(|b| format!("{:02x}", b)).collect::<String>();
    
    // 3. Verify checksum changed
    assert_ne!(original_checksum, new_checksum, "Checksum should have changed after corruption");
    
    // 4. Verify decryption fails due to corrupted payload (AES-GCM tag check)
    let header_bytes = &file_bytes[..4096];
    let payload_bytes = &file_bytes[4096..];
    let header_str = String::from_utf8_lossy(header_bytes);
    let header_json: serde_json::Value = serde_json::from_str(header_str.trim()).unwrap();
    let wrapped_dek_b64 = header_json["metadata"]["wrapped_dek"].as_str().unwrap();
    let wrapped_dek_bytes = crypto::from_base64(wrapped_dek_b64).unwrap();
    let dek_bytes = crypto::decrypt_payload_binary(&vault_key, &wrapped_dek_bytes).unwrap();
    
    let mut dek = [0u8; 32];
    dek.copy_from_slice(&dek_bytes);
    
    let decrypt_result = crypto::decrypt_payload_binary(&dek, payload_bytes);
    assert!(decrypt_result.is_err(), "Decryption should fail after payload corruption");
}
...
fn test_legacy_format_decryption() {
    ...
    let encrypted_payload = crypto::encrypt_payload_binary(&vault_key, &original_iq).unwrap();
    ...
    let header_bytes = &file_bytes[..4096];
    let payload_bytes = &file_bytes[4096..];
    let header_json: serde_json::Value = serde_json::from_str(String::from_utf8_lossy(header_bytes).trim()).unwrap();
    
    let decrypted_iq = if let Some(wrapped_dek_b64) = header_json["metadata"]["wrapped_dek"].as_str() {
        let wrapped_dek = crypto::from_base64(wrapped_dek_b64).unwrap();
        let dek_bytes = crypto::decrypt_payload_binary(&vault_key, &wrapped_dek).unwrap();
        let mut dek = [0u8; 32];
        dek.copy_from_slice(&dek_bytes);
    ...
    assert_eq!(decrypted_iq, original_iq, "Legacy decryption fallback failed");
}
