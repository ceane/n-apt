...
async fn test_server_status_endpoint() {
    ...
    let webauthn = WebauthnBuilder::new(rp_id, &rp_origin).unwrap().build().unwrap();
...
async fn test_auth_challenge_flow() {
    ...
    let session_store = SessionStore::new();
    let webauthn = WebauthnBuilder::new("localhost", &Url::parse("http://localhost").unwrap()).unwrap().build().unwrap();
    ...
    let verify_resp = server.post("/auth/verify").json(&serde_json::json!({
...
async fn test_auth_info_endpoint() {
    ...
    let state = Arc::new(AppState {
        ...
        webauthn: WebauthnBuilder::new("localhost", &Url::parse("http://localhost").unwrap()).unwrap().build().unwrap(),
        broadcast_tx,
        spectrum_tx,
        cmd_tx,
    });
