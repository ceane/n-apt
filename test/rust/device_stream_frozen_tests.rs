...
async fn test_device_reconnection_after_freeze() {
    ...
    shared_state.device_connected.store(false, Ordering::SeqCst);
    assert_eq!(shared_state.device_connected.load(Ordering::SeqCst), false);
...
async fn test_frequency_changes_during_freeze() {
    ...
    shared_state.pending_center_freq.store(initial_freq, Ordering::SeqCst);
    ...
    shared_state.pending_center_freq.store(new_freq, Ordering::SeqCst);
    shared_state.pending_center_freq_dirty.store(true, Ordering::SeqCst);
    
    // Verify frequency change is preserved
    assert_eq!(shared_state.pending_center_freq.load(Ordering::SeqCst), new_freq);
    assert!(shared_state.pending_center_freq_dirty.load(Ordering::SeqCst));
}
