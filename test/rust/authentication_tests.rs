...
fn test_authentication_condition_check() {
  ...
  assert!(
    !should_generate,
    "Should not generate mock data with no authenticated clients"
  );
  ...
  assert!(
    !should_generate,
    "Should not generate mock data when paused"
  );
  ...
  assert!(
    should_generate,
    "Should generate mock data with authenticated clients and not paused"
  );
}
...
fn test_mock_processor_creation() {
  ...
  assert!(
    result.is_ok(),
    "Mock data generation should work when called directly"
  );
