...
fn test_sdr_processor_frame_timing_stability() {
    ...
    for _ in 0..num_frames {
        ...
        let _ = processor.read_and_process_frame().expect("Failed to process frame");
        timings.push(start.elapsed());
    }

    let avg_time: Duration = timings.iter().sum::<Duration>() / (num_frames as u32);
...
fn test_fft_size_switch_to_max_first_frame_latency() {
    ...
    let _ = processor.read_and_process_frame().expect("Failed to warm max-size frame");

    let baseline_start = Instant::now();
    let _ = processor.read_and_process_frame().expect("Failed to process baseline max-size frame");
...
fn test_loop_interval_consistency_across_sizes() {
    ...
    for (fft_size, expected_fps) in test_cases {
        ...
        assert_eq!(target_fps, expected_fps, "Frame rate calculation mismatch for size {}", fft_size);
        ...
        let avg_interval: Duration = intervals.iter().skip(1).sum::<Duration>() / (intervals.len() as u32 - 1);
        println!("Size {}: Avg interval {:?}, Target {:?}", fft_size, avg_interval, target_duration);
        ...
        assert!(diff < jitter_limit, "Interval jitter too high for size {}: {:?}", fft_size, diff);
    }
}

#[test]
fn test_mock_sdr_sample_rate_correctness() {
    let processor = SdrProcessor::new_mock_apt().expect("Failed to create mock processor");
    assert_eq!(processor.get_sample_rate(), SAMPLE_RATE, "Mock SDR sample rate should match constants (3.2MHz)");
}
