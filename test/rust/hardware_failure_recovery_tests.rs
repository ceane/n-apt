...
mod failure_tests {
  ...
  async fn test_capture_recovery_on_hardware_malfunction() -> Result<()> {
    ...
    let capture_result = processor
      .stop_capture()
      .expect("Should return partial result even on failure");

    // 4. Verify the result is partial
    assert_eq!(capture_result.job_id, "failure-test-job");

    // Ensure at least some data was captured
    let total_iq_size: usize = capture_result
      .channels
      .iter()
      .map(|c| c.iq_data.len())
      .sum();
    let total_spectrum_size: usize = capture_result
      .channels
      .iter()
      .map(|c| c.spectrum_data.len())
      .sum();

    assert!(total_iq_size > 0, "Total IQ data should not be empty");
    assert!(
      total_spectrum_size > 0,
      "Total spectrum data should not be empty"
    );

    // Also verify duration is reasonable
    assert!(
      capture_result.duration_s < 9.0,
      "Duration should be significantly less than the requested 10s"
    );
    assert!(
      capture_result.duration_s > 0.01,
