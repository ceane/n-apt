...
const BuildOrchestrator = () => {
  ...
  const runBuildPipeline = async () => {
    app.update((state: BuildState) => ({ ...state, overall: 'running', startTime: Date.now() }));

    try {
      // Step 1: Cleanup
      app.update((state: BuildState) => ({
        ...state,
  ...
  app.view((state: BuildState) => (
    <Column p={2} gap={1}>
      ...
      <Row gap={1}>
        <Button
          id="start-build"
          label={state.overall === 'running' ? "Building..." : "Start Build"}
          intent={state.overall === 'running' ? "secondary" : "primary"}
        ...
        />
        <Spacer flex={1} />
        <Text>Total time: {(state.totalTime / 1000).toFixed(2)}s</Text>
      </Row>
