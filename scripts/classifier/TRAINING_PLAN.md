...
4. **Experiment search and deployed parity:** the bounded, predeclared multi-seed/L2 search and validation-only selection are implemented and tested on synthetic-only rows. Extend trained-artifact parity from TypeScript CPU to WGSL/WebGPU, including bias and out-of-range inputs. Synthetic tests prove plumbing only.
