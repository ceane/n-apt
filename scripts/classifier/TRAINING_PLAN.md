...
| Optimizer settings diverged and training returned no usable convergence details | Partly fixed; NumPy and PyTorch share weighted logits loss, explicit L2, and Adam settings; logistic uses backtracking; and the MLP uses group-disjoint training-monitor sessions, early stopping, and best-checkpoint restore when there are enough sessions. The CLI exposes seeds, optimizer/stopping settings, and optional candidate exports. Reports include a feature-dataset hash, split-session manifest, source revision/dirty state, runtime versions, configuration, and selected normalization. The bounded multi-seed/L2 search runner remains. |

## Implementation handoff: small, testable tasks

1. **Trainer correctness and reproducibility (core slice complete):** tests cover the original defects, finite-difference gradients, stable weighted logits loss, fit/export, abstentions, session-disjoint monitoring, checkpoint restore, report provenance, TypeScript inference parity for exported logistic and MLP candidates, and clear failure for an unavailable explicit MPS request. The trainer can emit both candidate artifacts. The default Python run passes 23 tests with three optional PyTorch tests skipped. A temporary CPython 3.14/PyTorch 2.14 environment passes all 23 tests, including monitored CPU MLP fitting; this host reports MPS unavailable. WGSL parity for trained artifacts and the predeclared multi-seed/L2 search runner remain. No real-capture training yet.
...
4. **Experiment search and deployed parity:** add the bounded, predeclared multi-seed/L2 candidate search and test its selection rule without reading the untouched evaluation split. Extend trained-artifact parity from TypeScript CPU to WGSL/WebGPU, including bias and out-of-range inputs. The current test uses synthetic-only rows and proves plumbing only.
