...
Reproducibility controls include `--seed`, `--monitor-fraction`, `--logistic-max-iter`, `--logistic-tolerance`, `--logistic-l2`, `--mlp-epochs`, `--mlp-patience`, `--mlp-min-delta`, `--mlp-l2`, and `--learning-rate`. These expose a single run's settings; the predeclared multi-seed/L2 search runner is still pending. The exported model/report are synthetic-only when every labeled train and validation row is explicitly marked `syntheticOnly`; synthetic runs prove the pipeline works, not signal accuracy.
