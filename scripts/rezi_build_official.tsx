...
// Create the Rezi application following the official pattern
const app = createNodeApp<{
  cleanup: { status: 'pending' | 'running' | 'success' | 'failed'; duration: number };
  frontend: { status: 'pending' | 'running' | 'success' | 'failed'; duration: number };
  wasm: { status: 'pending' | 'running' | 'success' | 'failed'; duration: number };
  backend: { status: 'pending' | 'running' | 'success' | 'failed'; duration: number };
...
// Define the view using the official Rezi pattern
app.view((s) => (
  <Column p={2} gap={1}>
    <Box border="single" p={1}>
      <Column gap={1}>
        ...
        <Row items="center" gap={1}>
          ...
          <Spacer flex={1} />
          <Text>
            {getStatusIcon(s.overall)} N-APT Build Orchestrator
          </Text>
        </Row>
        <Row items="center" gap={1}>
          <Text>│ p t │</Text>
    ...
    <Box border="single" p={1}>
      <Column gap={1}>
        ...
        <Row gap={2}>
          <Text>Cleanup:</Text>
          <Text>
            {getStatusIcon(s.cleanup.status)} {s.cleanup.status}
          </Text>
          {s.cleanup.duration > 0 && (
            <Text>({s.cleanup.duration}ms)</Text>
          )}
        </Row>
        ...
        <Row gap={2}>
          <Text>Frontend:</Text>
          <Text>
            {getStatusIcon(s.frontend.status)} {s.frontend.status}
          </Text>
          {s.frontend.duration > 0 && (
            <Text>({s.frontend.duration}ms)</Text>
          )}
        </Row>
        ...
        <Row gap={2}>
          <Text>WASM:</Text>
          <Text>
            {getStatusIcon(s.wasm.status)} {s.wasm.status}
          </Text>
          {s.wasm.duration > 0 && (
            <Text>({s.wasm.duration}ms)</Text>
          )}
        </Row>
        ...
        <Row gap={2}>
          <Text>Backend:</Text>
          <Text>
            {getStatusIcon(s.backend.status)} {s.backend.status}
          </Text>
          {s.backend.duration > 0 && (
            <Text>({s.backend.duration}ms)</Text>
          )}
        </Row>
      </Column>
    </Box>
    ...
    <Row gap={1}>
      ...
      />
      <Spacer flex={1} />
      <Text>Total time: {((Date.now() - s.startTime) / 1000).toFixed(2)}s</Text>
    </Row>
...
// Handle keyboard shortcuts using the official pattern
app.keys({
  q: () => app.stop(),
  'ctrl-c': () => app.stop(),
});
