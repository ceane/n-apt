...
export type CliToolDecision =
  | { allowed: true }
  | {
      allowed: false;
      reason: "unknown" | "blocked" | "mutation_requires_opt_in";
    };

export function evaluateCliToolRequest(
  name: string,
  ...
  if (!isToolAllowedForCli(name, allowMutations))
    return { allowed: false, reason: "mutation_requires_opt_in" };
  return { allowed: true };
}
...
export function printAgentCapabilities(json: boolean) {
  const value = {
    ...
    routes: agentCapabilities.routes,
  ...
  if (json) console.log(JSON.stringify(value, null, 2));
    ...
    value.routes.forEach((route) =>
      console.log(
        `${route.path.padEnd(22)} ${route.status.padEnd(14)} ${route.label}`,
      ),
    );
...
export async function fetchAgentMarkdown(baseUrl: string, route: string) {
  ...
  if (!response.ok)
    throw new Error(`Markdown request failed: HTTP ${response.status}`);
  return {
    route,
...
export async function executeAgentTool(
  ...
  if (!decision.allowed)
    throw new Error(`Tool ${name} rejected: ${decision.reason}`);
  const response = await fetch(
    `${baseUrl.replace(/\/$/, "")}/api/webmcp/execute`,
    {
      method: "POST",
      headers: {
  ...
  if (!response.ok)
    throw new Error(
      result.error ?? `Tool request failed: HTTP ${response.status}`,
    );
